...
def rbf(x_1, x_2, sigma=1.):
    ...
    for i in range(0, x_1.shape[0]):
        for j in range(0, x_2.shape[0]):
            distances[i, j] = np.exp(torch.norm(x_1[i] - x_2[j], p = 2) / (-2 * sigma**2))
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                preds = self.betas.T @ k_batch.T + self.bias ### YOUR CODE HERE # get the matrix product using SVM parameters: self.betas and self.bias
                preds = preds.flatten()
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            return (self.betas.T @ K.T + self.bias).flatten()
